...
install_main_config() {
	...
	if [[ -f "${MAIN_CONFIG}" ]]; then
		if grep -qE 'autogenerated config|autogenerated' "${MAIN_CONFIG}"; then
			local backup_path="${MAIN_CONFIG}.autogenerated.$(date +%Y%m%d-%H%M%S).bak"
			mv "${MAIN_CONFIG}" "${backup_path}"
			printf '%s\n' "Backed up autogenerated config to ${backup_path}."
		else
			printf '%s\n' "Keeping existing user config at ${MAIN_CONFIG}."
			return 0
		fi
	fi
